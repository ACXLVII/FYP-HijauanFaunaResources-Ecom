...
export default function ProductCard({ product }) {
  ...
  const getImageSrc = (imageData) => {
    ...
    const imageType = imageString.match(/^data:image\/(\w+);base64,/) 
      ? imageString.split(';')[0].split('/')[1] 
      : 'jpeg';
  ...
  const imageSrc = product.images?.[0] ? getImageSrc(product.images[0]) : '/images/shop/plants.jpg';
    ...
    >
      ...
      <div className="p-4 lg:p-8">
        ...
        <button
          ...
          onClick={() => product.inStock && (window.location.href = `/shop/decorative_plants/product/${product.doc_id}`)}
